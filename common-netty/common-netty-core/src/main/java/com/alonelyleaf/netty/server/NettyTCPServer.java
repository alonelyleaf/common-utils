...
import com.alonelyleaf.netty.api.exception.ServiceException;
...
public abstract class NettyTCPServer implements Service {
    ...
    private void createServer(Listener listener, EventLoopGroup boss, EventLoopGroup work, ChannelFactory<? extends ServerChannel> channelFactory) {
        ...
        try {
             ...
             * 绑定端口并启动去接收进来的连接
             */
            InetSocketAddress address = (host == null || host.isEmpty()) ? new InetSocketAddress(port) : new InetSocketAddress(host, port);
